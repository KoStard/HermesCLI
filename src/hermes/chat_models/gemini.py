...
class GeminiModel(ChatModel):
    ...
    def send_history(self, messages) -> Generator[str, None, None]:
        history = [genai.types.ContentDict(role=msg['role'], parts=[msg['content']]) for msg in messages]
        chat = genai.GenerativeModel('gemini-1.5-pro-exp-0801').start_chat(history=history[:-1])
        response = chat.send_message(history[-1]['content'], stream=True, safety_settings={
            HarmCategory.HARM_CATEGORY_HARASSMENT: HarmBlockThreshold.BLOCK_NONE,
            HarmCategory.HARM_CATEGORY_HATE_SPEECH: HarmBlockThreshold.BLOCK_NONE,
            HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT: HarmBlockThreshold.BLOCK_NONE,
            HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT: HarmBlockThreshold.BLOCK_NONE,
