...
def test_add_argument(url_provider):
...
@patch('requests.get')
def test_fetch_url_content_success(mock_get, url_provider):
    mock_response = Mock()
...
@patch('requests.get')
def test_fetch_url_content_retry(mock_get, url_provider):
    mock_response = Mock()
    mock_response.text = '<html><body><h1>Test</h1></body></html>'
    mock_response.raise_for_status.return_value = None
...
@patch('requests.get')
def test_fetch_url_content_failure(mock_get, url_provider):
    mock_get.side_effect = RequestException

    with pytest.raises(RequestException):
        url_provider.fetch_url_content('http://example.com')

def test_load_context(url_provider):
    args = Mock()
...
def test_add_to_prompt(url_provider):
...
def test_html_to_markdown(url_provider):
