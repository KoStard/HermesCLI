...
class ContentProcessor:
    ...
    def _process_lines(
        ...
        while i < len(lines):
            i = self._process_line(lines, i, path_tracker, section_path,
                                  new_content, mode, is_preface, updated_lines)
    ...
    def _process_line(
        ...
        return self._handle_section_match(
            lines, i, header.level, new_content, mode, is_preface, updated_lines)
    ...
    def _handle_preface_update(
        self, lines: list[str], i: int, new_content: str, mode: str, updated_lines: list[str]
    ) -> int:
    ...
    def _append_preface_content(
        self, lines: list[str], i: int, next_idx: int, new_content: str, updated_lines: list[str]
    ) -> None:
        """Append content to preface section."""
        updated_lines.extend(lines[i:next_idx])
        updated_lines.append(new_content)

    def _update_preface_content(
        self, line: str, new_content: str, updated_lines: list[str]
    ) -> None:
        """Update preface content."""
        updated_lines.extend([line, new_content])

    def _handle_append(
        self, lines: list[str], i: int, level: int, new_content: str, updated_lines: list[str]
    ) -> int:
