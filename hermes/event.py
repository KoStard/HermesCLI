...
class FileEditEvent(EngineCommandEvent):
    ...
    content: str
    mode: (
        str  # 'create', 'append', 'update_markdown_section', 'append_markdown_section'
    )
    submode: str = None  # Optional, only for specific use cases
    section_path: list[str] = (
        None  # For markdown section updates, e.g. ['Introduction', 'Overview', '__preface']
    )
...
class RawContentForHistoryEvent(Event):
    ...
    def from_json(json_data: dict) -> "RawContentForHistoryEvent":
        ...
        if content_type == "text":
            return RawContentForHistoryEvent(
                content=TextMessage.from_json(json_data["content"])
            )
        elif content_type == "text_generator":
            return RawContentForHistoryEvent(
                content=TextGeneratorMessage.from_json(json_data["content"])
            )
        elif content_type == "thinking_and_response_generator":
            return RawContentForHistoryEvent(
                content=ThinkingAndResponseGeneratorMessage.from_json(
                    json_data["content"]
                )
            )
        else:
