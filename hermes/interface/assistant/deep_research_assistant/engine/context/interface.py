...
class DeepResearcherInterface:
    ...
    def _gather_dynamic_section_data(
        ...
        # Budget
        all_data[BudgetSectionData] = BudgetSectionData(
        ...
        all_data[ProblemHierarchyData] = ProblemHierarchyData.from_filesystem_and_node(
            fs=self.file_system, target_node=target_node
        )

        # Criteria - Use factory method
        all_data[CriteriaSectionData] = CriteriaSectionData.from_node(
            target_node=target_node
        )

        # Subproblems - Use factory method
        all_data[SubproblemsSectionData] = SubproblemsSectionData.from_node(
            target_node=target_node
        )
        ...
        all_data[KnowledgeBaseData] = KnowledgeBaseData.from_knowledge_base(
        ...
        ordered_data = [
            all_data[section_type]
            for section_type in DYNAMIC_SECTION_ORDER
            if section_type in all_data
        ]

        # Sanity check: Ensure all expected sections were populated
        if len(ordered_data) != len(DYNAMIC_SECTION_ORDER):
            print(
                "Warning: Mismatch between expected dynamic sections and gathered data."
            )
    ...
    def _collect_artifacts_recursively(
        ...
        for name, artifact in node.artifacts.items():
            ...
            is_owned_by_ancestor = (
                node in self._get_parent_chain(current_node)[:-1]
            )  # Exclude current node itself

            # Determine if the artifact *should* be visible based on ownership/ancestry
            # External files are always visible conceptually.
            should_be_visible = (
                artifact.is_external or is_owned_by_current or is_owned_by_ancestor
            )
