...
class ModelFactory:
    ...
    def get_model(
        ...
        matching_classes = [
            m for m in self.model_classes if m.get_provider().upper() == provider
        ]

        if not matching_classes:
            raise ValueError(
                f"Unsupported model provider: {provider}, supported providers: {list(set(m.get_provider().upper() for m in self.model_classes))}"
            )
