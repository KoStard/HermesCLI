...
class UserControlPanel(ControlPanel):
    def __init__(self, *, 
        ...
        self._register_command(ControlPanelCommand(command_label="/clear", command_key="clear_history", description="Clear the conversation history", parser=lambda _: ClearHistoryEvent(), priority=99, visible_from_cli=False)) # Clear history should be the first command, we'll clear then do the rest
        self._register_command(ControlPanelCommand(command_label="/image", command_key="add_image", description="Add image to the conversation", parser=lambda line: MessageEvent(ImageMessage(author="user", image_path=line))))
        self._register_command(ControlPanelCommand(command_label="/image_url", command_key="add_image_url", description="Add image from url to the conversation", parser=lambda line: MessageEvent(ImageUrlMessage(author="user", image_url=line))))
        self._register_command(ControlPanelCommand(command_label="/audio", command_key="add_audio", description="Add audio to the conversation", parser=lambda line: MessageEvent(AudioFileMessage(author="user", audio_filepath=line))))
        self._register_command(ControlPanelCommand(command_label="/video", command_key="add_video", description="Add video to the conversation", parser=lambda line: MessageEvent(VideoMessage(author="user", video_filepath=line))))
        self._register_command(ControlPanelCommand(command_label="/pdf", command_key="add_pdf", description="Add pdf to the conversation. After the PDF path, optionally use {<page_number>, <page_number>:<page_number>, ...} to specify pages.", parser=lambda line: MessageEvent(EmbeddedPDFMessage.build_from_line(author="user", raw_line=line))))
        self._register_command(ControlPanelCommand(command_label="/textual_file", command_key="add_textual_file", description="Add text file to the conversation. Supported: plain textual files, PDFs, DOCs, PowerPoint, Excel, etc.", parser=lambda line: MessageEvent(TextualFileMessage(author="user", text_filepath=line)), default_on_cli=True))
        self._register_command(ControlPanelCommand(command_label="/url", command_key="add_url", description="Add url to the conversation", parser=lambda line: MessageEvent(UrlMessage(author="user", url=line))))
        self._register_command(ControlPanelCommand(command_label="/save_history", command_key="save_history", description="Save history to a file", parser=lambda line: SaveHistoryEvent(line), visible_from_cli=False))
        self._register_command(ControlPanelCommand(command_label="/load_history", command_key="load_history", description="Load history from a file", parser=lambda line: LoadHistoryEvent(line), priority=98))
        self._register_command(ControlPanelCommand(command_label="/text", command_key="add_text", description="Add text to the conversation", parser=lambda line: MessageEvent(TextMessage(author="user", text=line, is_directory_entered=True))))
        self._register_command(ControlPanelCommand(command_label="/exit", command_key="exit_app", description="Exit the application", parser=lambda _: ExitEvent(), priority=-100))  # Run exit after running any other command
        self._register_command(ControlPanelCommand(command_label="/tree", command_key="generate_tree", description="Generate a directory tree", parser=self._parse_tree_command))
    ...
    def break_down_and_execute_message(self, message: Message) -> Generator[Event, None, None]:
        ...
        for line in peekable_generator:
            ...
            if matching_command:
                ...
                command_priority = self._all_commands[matching_command].priority
                command_parser = self._all_commands[matching_command].parser
    ...
    def convert_cli_arguments_to_text(self, parser: ArgumentParser, args: Namespace) -> str:
        ...
        for arg, value in commands_group:
            if arg not in self.disabled_tools:
                if arg != "prompt":
                    for v in value:
